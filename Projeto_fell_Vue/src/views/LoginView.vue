...
async function handleLogin() {
  ...
  try {
    const response = await axios.post('/app/controller/auth_controller/process_login.php', {
      email: email.value,
    ...
    if (response.data.success) {
      ...
      setTimeout(() => {
        // router.push('/home') // Redirecionar para home quando existir
        alert('Login realizado! Redirecionando para Home...')
      }, 1000)
    } else {
      showAlert(response.data.message || 'Erro ao realizar login.', 'error')
    }
  } catch (error) {
...
<template>
  <div class="login-container">
    <!-- background accents (blobs animados) -->
    ...
    <div class="glass-card">
      ...
      <form @submit.prevent="handleLogin" class="login-form">
        ...
        <div class="mb-3">
          <label for="password" class="form-label text-muted small fw-bold">Senha</label>
          <input 
            type="password" 
            id="password" 
            v-model="password" 
        ...
        <div class="text-end mb-4">
          <router-link to="/recover" class="small-link">Esqueceu a senha?</router-link>
        </div>
      ...
      <div class="register-area mt-4 text-center">
        <p class="text-muted small">
          Ainda não tem conta? 
          <router-link to="/register" class="fw-bold text-primary-custom">Registre-se</router-link>
        </p>
      </div>
    </div>
  </div>
