...
export const globalSearch = async (req: Request, res: Response) => {
  ...
  try {
    const users = await prisma.$queryRaw`SELECT user_id, first_name, last_name, profile_pic FROM user WHERE first_name LIKE ${term} OR last_name LIKE ${term} LIMIT 10`;
